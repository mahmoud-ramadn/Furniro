...
const stripePromise = loadStripe("pk_test_51Qi1ltG1Yc7r0GlRGtv1njDWunCxy3QmSE4A8BBj6o6gcORsLZ3LM1617YYlZyhwjXPyCM1hpQvbxQEfgJ3g03li00CMDLYU9n");
                        ...
                        {FormInput.map((field, index) => (
                            <div key={index} className="md:h-[121px] flex flex-col justify-between">
                                ...
                                {errors[field.id as keyof Formcheckout] && (
                                    <p className="text-sm text-danger-500">{errors[field.id as keyof Formcheckout]?.message}</p>
                                )}
                            </div>
                        ))}
                    </div>
